...
# Fonte padrão do Linux (funciona no Railway)
...
# Processa logs de forma segura (pode chamar manualmente)
def process_logs():
    db = load_db()
    ...
    for arquivo in os.listdir(RESULTADOS_DIR):
        ...
        with open(caminho, "r", encoding="utf-8", errors="ignore") as f:
            conteudo = f.read()
        # TIMES
        teams = re.findall(r"TeamName:\s*(.*?)\s+Rank:\s*(\d+)\s+KillScore:\s*(\d+)\s+RankScore:\s*(\d+)\s+TotalScore:\s*(\d+)",
...
@app.route("/")
def home():
    # NÃO processar logs aqui para não travar Railway
    db = load_db()
...
# Exportar tabela (exemplo simplificado)
@app.route("/export")
def export():
    try:
        db = load_db()
